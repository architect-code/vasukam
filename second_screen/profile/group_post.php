...
pv.vote as vote
FROM post_data pd JOIN grps gp ON pd.where_id = gp.grp_id
JOIN users us ON pd.origin_id = us.user_id
LEFT JOIN post_vote pv ON pv.post_id = pd.post_id
LEFT JOIN users us2 ON pv.user_id = us2.user_id
WHERE gp.grp_id = :gi AND (us2.user_id = :ui OR us2.user_id IS NULL)
ORDER BY pd.post_time DESC";
     $stmt_group_post = $pdo->prepare($group_post_query);
     $stmt_group_post->execute(array(
        ":ui"=>$user_main,
        ":gi"=>$group_main,
    ));

    $result=$stmt_group_post->fetchAll(PDO::FETCH_ASSOC);
    if(count($result) == 0){
        echo("
