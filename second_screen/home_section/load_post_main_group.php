...
$user_main = $_SESSION["account"];
    ...
    JOIN users us3 ON pd.owner_id  = us3.user_id
    LEFT JOIN ask_price ap ON ap.post_id = pd.post_id
    LEFT JOIN post_vote pv ON (pd.post_id = pv.post_id AND pv.user_id = us1.user_id)
...
$stmt_group = $pdo->prepare($post_query_grp);
...
$result=$stmt_group->fetchAll(PDO::FETCH_ASSOC);
if(count($result)==0){
    echo("
    ...
    Greetings from Vasukam, to see post here join some new groups
    </div>
